...
session_start();
 ...
 if(isset($_REQUEST['psubmit'])){
  ...
  if(($_REQUEST['cname'] == "") || ($_REQUEST['cadd'] == "") || ($_REQUEST['pname'] == "") || ($_REQUEST['pquantity'] == "")
   ...
   $msg = '<div class="alert alert-warning text-center mr-5 ml-5 mt-2" role="alert">* All Fileds Are Required </div>';
  } else {
    // Assigning User Values to Variable for update
    $pid = $_REQUEST['pid'];
    $pava = ($_REQUEST['pava'] - $_REQUEST['pquantity']); //avaialble quantity will get - from purchase quantity and get updated

    // Assigning User Values to Variable for insert
    $custname = $_REQUEST['cname'];
    $custadd = $_REQUEST['cadd'];
    $cpname = $_REQUEST['pname'];
    $cpquantity =$_REQUEST['psellingcost'];
    ...
    if($conn->query($sqlin) == TRUE){
      ...
      $genid = mysqli_insert_id($conn); //mysqli_insert_id it is a builtin function which will capture id and store into  var $genid.
      session_start();
      $_SESSION['myid'] = $genid; // after that we will store it into var myid and redirect it into 'productsellsuccess.php
 ...
 ?>
  <form action="" method="POST">
    ...
    <div class="form-group">
      ...
      <input type="number" min="1" max="10" class="form-control" id="pquantity" name="pquantity" onkeypress="isInputNumber(event)">
    </div>
    <div class="form-group">
    ...
    <div class="text-center">
      <button type="submit" class="btn butn-colour" id="psubmit" name="psubmit">Submit</button>
