...
session_start();
 ...
 if(isset($_REQUEST['empupdate'])){
  ...
  if(($_REQUEST['empName'] == "") || ($_REQUEST['empCity'] == "") || ($_REQUEST['empMobile'] == "") || ($_REQUEST['empEmail'] == "")){
   ...
   $msg = '<div class="alert alert-warning mr-5 ml-5 mt-2" role="alert">* All Fileds Are Required </div>';
  } else {
  ...
  $sql = "UPDATE technician_tb SET empName = '$eName', empCity = '$eCity', empMobile = '$eMobile', empEmail = '$eEmail' WHERE empid = '$eId'";
    if($conn->query($sql) == TRUE){
     ...
     $msg = '<div class="alert alert-success mr-5 ml-5 mt-2 text-center" role="alert"> Updated Successfully </div>';
    } else {
     // below msg display on form submit failed
     $msg = '<div class="alert alert-danger mr-5 ml-5 mt-2 text-center" role="alert"> Unable to Update </div>';
    }
  }
  }
 ?>
 ...
 ?>
  <form action="" method="POST">
    ...
    <div class="text-center">
      <button type="submit" class="btn butn-colour" id="empupdate" name="empupdate">Update</button>
