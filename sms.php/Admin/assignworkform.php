<?php    
if(session_id() == '') {  //if session is not start then it will get start otherwise it will ignore it.
  session_start();
}
...
//agar view pr click kiye or view is set then selected id data will be get displayed.
 if(isset($_REQUEST['view'])){
  $sql = "SELECT * FROM submitrequest_tb WHERE request_id = {$_REQUEST['id']}"; //request_id is databse table name & ['id'] is input field name which is on request.php form.
...
if(isset($_REQUEST['close'])){
 ...
 if(isset($_REQUEST['assign'])){
  ...
  if(($_REQUEST['request_id'] == "") || ($_REQUEST['request_info'] == "") || ($_REQUEST['requestdesc'] == "")
   ...
   $msg = '<div class="alert alert-warning col-sm-6 ml-5 mt-2" role="alert"> * All Fileds Are Required </div>';
  } else {
    ...
    //after asigning values into variables..we will insert those variable values into db assignwork_tb.
